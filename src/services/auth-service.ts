...
import { SignInPayload, SignUpPayload } from "@/features/auth/schemas";
...
export async function validateCredential(payload: SignInPayload) {
  ...
  if (!user.emailVerified) {
    if (await isVerifyEmailSent(email)) {
      throw new ApplicationError("Verify your email to sign in");
    }
...
export async function forgotPassword(email: string) {
  const user = await findUserByEmail(email);
  if (!user) throw new ValidationError({ email: ["Not exists!"] });
